        ...
        foreach ($data as $key => $item) {
            ...
            $html .= '<tr>
							<td style="text-align:center;">' . $n++ . '
								<input class="form-control" type="hidden" name="id_form" id="id_form" value=' . $item['id_form'] . '/>
							</td>
							<td>' . $item['kode_barang_dtl'] . '
        ...
        $html .= "<script>$('.hapus-bom').click(function(){
						...
						$.ajax({
							...
							success:function(data){
								$('#detail_input_barang').html(data).show();
							 }
						  });
					  });
				     </script>";
        ...
        if (mysql_num_rows($result) > 0) {
            while ($res = mysql_fetch_array($result)) {
                ...
                if (!isset($_SESSION['data_barang_edit'])) {
                    $array[] = array("id" => $res['id_bom_dtl'], "kode_barang_dtl" => $res['kode_barang_dtl'], "satuan_dtl" => $res['satuan_dtl'], "qty_satuan_dtl" => $res['qty_satuan_dtl'], "keterangan_dtl" => $res['keterangan_dtl'], "id_form" => $res['id_form']);
                } else {
                    $array = $_SESSION['data_barang_edit'];
                    $array[] = array("id" => $res['id_bom_dtl'], "kode_barang_dtl" => $res['kode_barang_dtl'], "satuan_dtl" => $res['satuan_dtl'], "qty_satuan_dtl" => $res['qty_satuan_dtl'], "keterangan_dtl" => $res['keterangan_dtl'], "id_form" => $id_form);
                }
            }
        }
...
if (isset($_REQUEST['func']) and @$_REQUEST['func'] == "save") {
    ...
    for ($x = 0; $x < $jumlah2; $x++) {
        ...
        $mySql2 = "INSERT INTO harga_inventori SET
						kode_inventori			='" . $kode_inventori . "',
						harga					='" . str_replace(",", null, $harga[$x]) . "',
						diskon					='" . str_replace(",", null, $diskon[$x]) . "',
...
if (isset($_REQUEST['func']) and @$_REQUEST['func'] == "update") {
    ...
    if (isset($_SESSION['data_barang_edit'])) {
        ...
        $query            = "SELECT * FROM bom WHERE kode_inventori='" . $kode_inventori . "'";
        $result            = mysql_query($query);
        $array = array();
        if (mysql_num_rows($result) > 0) {
            while ($res = mysql_fetch_array($result)) {
                ...
                if (!isset($_SESSION['data_barang_edit'])) {
                    $array[] = array("id" => $res['id_bom'], "kode_barang_dtl" => $res['kode_barang_dtl'], "satuan_dtl" => $res['satuan_dtl'], "qty_satuan_dtl" => $res['qty_dtl'], "keterangan_dtl" => $res['keterangan_dtl'], "id_form" => $res['id_form']);
                } else {
                    $array = $_SESSION['data_barang_edit'];
                    $array[] = array("id" => $res['id_bom'], "kode_barang_dtl" => $res['kode_barang_dtl'], "satuan_dtl" => $res['satuan_dtl'], "qty_satuan_dtl" => $res['qty_dtl'], "keterangan_dtl" => $res['keterangan_dtl'], "id_form" => $id_form);
                }
            }
        }
        $_SESSION['data_barang_edit'] = $array;
    ...
    foreach ($array as $key => $item) {
        ...
        $hapusbomlama = mysql_query("DELETE FROM bom WHERE kode_inventori = '" . $kode_inventori . "' ");
    ...
    $hapusbomtmp = mysql_query("DELETE FROM bom_tmp");
    ...
    for ($x = 0; $x < $jumlah2; $x++) {
        ...
        $checkup = "SELECT * FROM harga_inventori WHERE kode_inventori = '" . $kode_inventori . "' and kode_kategori_pelanggan	='" . $kode_kategori_pelanggan[$x] . "'";

        $checkup = mysql_query($checkup);

        if (mysql_num_rows($checkup) > 0) {

            $mySql2 = "UPDATE harga_inventori SET harga	 = '" . str_replace(",", null, $harga[$x]) . "', diskon = '" . str_replace(",", null, $diskon[$x]) . "' WHERE kode_inventori = '" . $kode_inventori . "' and kode_kategori_pelanggan	='" . $kode_kategori_pelanggan[$x] . "'";
        } else {
            $mySql2 = "INSERT INTO harga_inventori (harga, diskon, kode_inventori, kode_kategori_pelanggan) VALUES ('" . str_replace(",", null, $harga[$x]) . "', '" . str_replace(",", null, $diskon[$x]) . "', '" . $kode_inventori . "', '" . $kode_kategori_pelanggan[$x] . "') ";
        }
        // die($mySql2);
        $query2 = mysqli_query($con, $mySql2);
    }
