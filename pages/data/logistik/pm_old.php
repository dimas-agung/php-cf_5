				                ...
				                <div class="col-lg-4" >
				                    <select id="kode_gudang_tujuan" name="kode_gudang_tujuan" class="select2" style="width: 100%;">
                                        <option value="">-- Pilih Gudang Tujuan --</option>
                                        	<?php
                                                while($row_gudang = mysql_fetch_array($q_gudang_tujuan)) {
                                                    if (strtolower($row_gudang['nama_gudang']) === 'produksi') {
                                                    ?>

                                                <option value="<?php echo $row_gudang['kode_gudang'];?>"><?php echo $row_gudang['kode_gudang'].'&nbsp;&nbsp;||&nbsp;&nbsp;'.$row_gudang['nama_gudang'];
                                                ?> </option>
                                            <?php } } ?>
                                    </select>
				                </div>
...
<script>
    $(document).ready(function (e) {
         $("#saveForm").on('submit',(function(e) {
            ...
            var kode_gudang_asal = $("#kode_gudang_asal").val();
            if(kode_gudang_asal == 0 ) {
                $("#kode_gudang_asal").focus();
                notifError("<p>Gudang Asal tidak boleh kosong!!!</p>");
                return false;
            }
            var kode_gudang_tujuan = $("#kode_gudang_tujuan").val();
            if(kode_gudang_tujuan == 0 ) {
                $("#kode_gudang_tujuan").focus();
            ...
            if(grand_total >= 0) {
                ...
                $.ajax({
                    ...
                    success: function(html)
                    {
                        var msg = html.split("||");
                        if(msg[0] == "00") {
                            window.location = '<?=base_url()?>?page=logistik/pm_list&pesan='+msg[1];
                        } else {
                            notifError(msg[1]);
                        }
                        $(".animated").hide();
