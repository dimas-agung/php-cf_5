...
<section class="content">
    <div class="col-md-10">
      <?php
        $prev = mysql_fetch_array($q_gk_prev); {
        if (isset($prev['id_gk_hdr'])){
      ?>
          <a class="btn btn-warning" href="<?=base_url()?>?page=keuangan/gk_track&action=track&halaman= TRACK GIRO KELUAR&kode_gk=<?=$prev['kode_gk']?>">
      ...
      <?php
        ...
        $next = mysql_fetch_array($q_gk_next); {
        if (isset($next['id_gk_hdr'])){
      ?>
          &nbsp;<a class="btn btn-warning" href="<?=base_url()?>?page=keuangan/gk_track&action=track&halaman= TRACK GIRO KELUAR&kode_gk=<?=$next['kode_gk']?>">
      ...
      ?>
        ...
        ?>
          ...
          <a href="<?=base_url()?>?page=keuangan/gk_back&action=pengembalian&halaman= PENGEMBALIAN GIRO KELUAR&kode_gk=<?=$stat['kode_gk']?>" class="btn pengembalian <?php echo $status2?>" style="color: white"><i class=" fa fa-retweet"></i> PENGEMBALIAN</a>
      </div>
      <div class="col-md-2">
        <a href="#modalAddItem" class="btn btn-info"  data-toggle="modal"><i class=" fa fa-book"></i> JURNAL</a> 
        &nbsp;
        <a href="<?=base_url()?>?page=keuangan/gk&halaman= GIRO KELUAR" class="btn btn-danger pull-right" ><i class=" fa fa-reply"></i> BACK</a>
      </div>
</section>
...
<div class="box box-info">
    <div class="box-body">
		...
		<?php
			// HEADER
			$res_hdr = mysql_fetch_array($q_gk_hdr); 
                ...
                <div class="panel-body">
                  <div class="form-horizontal">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-lg-4">
                                    <input type="text" class="form-control" name="kode_gk" id="kode_gk" placeholder="Kode GK ..." readonly value="<?=$res_hdr['kode_gk']?>">
                                </div>
                                              ...
                                              <table id="simple-table" class="table  table-bordered table-hover">
                                                ...
                                                <tbody>
                                                    <?php
                                                        ...
                                                        while($res_dtl = mysql_fetch_array($q_gk_dtl)) { 
                                                        $total = $res_dtl['nominal_bayar'];
                                                        $total1 = $res_dtl['nominal_pelunasan'];
                                                 ...
                                                 <?php
                                                    ...
                                                    ?>
                                                      <tr>
                                                          ...
                                                          <td><?=strftime("%A, %d %B %Y", strtotime($res_giro['tgl_giro']))?></td>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                 <h4 class="modal-title">Kode GK : <?=$res_hdr['kode_gk']?></h4>
            </div>
                            ...
                            <div class="box-body">
                                <table class="" rules="all">
                                  ...
                                  <tbody>
                                      ...
                                      ?>
                                      <tr>
                                        <td colspan="4" style="text-align:center">Jurnal dengan Kode GK ini telah dibatalkan !!</td>
                                      </tr>
                                      <?php 
                                        }
                                      ?>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header" style="background-color: #9c0303">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                 <h4 class="modal-title" style="color: white">Pembatalan Kode GK : <?=$res_hdr['kode_gk']?></h4>
            </div>
            ...
            <form id="formPembatalan" method="post" enctype="multipart/form-data">             
                <div class="modal-body" style="min-height: 50px;">
                    <div class="control-group">
                      ...
                      <input type="hidden" class="form-control" name="kode_gk_batal" id="kode_gk_batal" value="<?=$res_hdr['kode_gk']?>">
                ...
                <div class="modal-footer" style="background-color: #9a24241a; border-top: 1px solid #ab5d5d;">
                  <button type="submit" name="batal" id="batal" class="btn btn-default" onclick="return confirm('Anda yakin akan membatalkan Giro Keluar ini ?')">
...
$(document).ready(function (e) {
    $("#formPembatalan").on('submit',(function(e) {
        ...
        if(alasan_batal != "") {           
            $(".animated").show();
            $.ajax({
                url: "<?=base_url()?>ajax/j_gk.php?func=pembatalan",
                type: "POST",
                ...
                success: function(html)
                {
                    var msg = html.split("||");
                    if(msg[0] == "00") {
                        window.location = '<?=base_url()?>?page=keuangan/gk&halaman= GIRO KELUAR&pembatalan='+msg[1];
                    } else {
                        notifError(msg[1]);
                    }
                    $(".animated").hide();
