<?php
	include "pages/data/script/gt.php";
    ...
    <div class="box-body">
    		...
    		<?php if (isset($_GET['pesan'])){ ?>
				<div class="form-group" id="form_report">
				  <div class="alert alert-success alert-dismissable">
					  ...
					  Kode GT :  <a href="<?=base_url()?>?page=keuangan/gt_track&halaman= TRACK TOLAKAN GIRO&action=track&kode_gt=<?=$_GET['pesan']?>" target="_blank"><?=$_GET['pesan'] ?></a>  Berhasil Di posting
				  </div>
				</div>
			...
			<?php }else if (isset($_GET['pembatalan'])){ ?>
				<div class="form-group" id="form_report">
				  <div class="alert alert-dismissable" style="background-color: #9c0303cc; color: #f9e6c3">
					  ...
					  Pembatalan Kode GT :  <a style="color: white" href="<?=base_url()?>?page=keuangan/gt_track&halaman= TRACK TOLAKAN GIRO&action=track&kode_gt=<?=$_GET['pembatalan']?>" target="_blank"><?=$_GET['pembatalan'] ?></a>  Berhasil Di batalkan
				  </div>
				</div>
			<?php }else if (isset($_GET['pengembalian'])){ ?>
				<div class="form-group" id="form_report">
				  <div class="alert alert-dismissable" style="background-color: #efb25ade; font-family: Trebuchet MS;">
					  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
					  <b>Pengembalian Kode GT :  <a href="<?=base_url()?>?page=keuangan/gt_track&halaman= TRACK TOLAKAN GIRO&action=track&kode_gt=<?=$_GET['pengembalian']?>" target="_blank"><?=$_GET['pengembalian'] ?></a>  Berhasil Di posting</b>
				  </div>
				</div>
			<?php } ?>
		...
		<div class="tabbable">
			<ul class="nav nav-tabs padding-12 tab-color-blue background-blue" id="myTab4">
				<li <?=$class_form?>>
					<a data-toggle="tab" href="#menuFormGt">Form Tolakan Giro</a>
				</li>
                <li <?=$class_tab?>>
					<a data-toggle="tab" href="#menuListGt">List Tolakan Giro</a>
				</li>
            </ul>
					...
					<?=$class_pane_form?> >
						<div class="col-lg-12">
							<div class="panel panel-default">
								<div class="panel-body">
                                	<div class="form-horizontal">
			                         ...
			                         <div class="form-group">
			                         	<label class="col-lg-2 col-sm-2 control-label" style="text-align:left">Kode GT</label>
			                         	<div class="col-lg-4">
			                             	<input type="text" class="form-control" name="kode_gt" id="kode_gt" placeholder="Auto..." readonly value="">
			                         	</div>
								...
								<div class="form-group">
	                            	<label class="col-lg-2 col-sm-2 control-label" style="text-align:left">Kode Pelunasan Giro</label>
	                                <div class="col-lg-4">
								        <select id="kode_pg" name="kode_pg" class="select2" disabled>
								...
								<div class="form-group">
									...
									<div class="col-lg-4">
                                            ...
                                            <table id="" class="" rules="all">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th>Nominal Giro Tolakan</th>
                                                        <th>Nominal Pengganti</th>
                                                        <th>Nominal Sisa</th>
                                                        <th>Metode Pengganti</th>
                                                        <th>Keterangan</th>
                                                    </tr>
                                                </thead>
                                                <tbody id="detail_input_gt">
                                                	<tr>
									 ...
									 <div class="form-group col-md-6">
                                            ...
                                            <a href="<?=base_url()?>?page=keuangan/gt&halaman= TOLAKAN GIRO" class="btn btn-danger"><i class=" fa fa-reply"></i> Batal</a>
               						 </div>
								...
								<div class="panel-body">
									...
									<!-- <form action="" method="post" >
			                         	...
			                         	<div class="col-xs-12 col-md-4" style="margin-bottom:3mm">
                                      		<div class="input-group">
				                             	...
				                             	<input type="text" autocomplete="off" class="form-control" name="kode_gt" id="kode_gt" placeholder="Kode gt ..." value="<?php

													if(empty($_POST['kode_gt'])){
														echo "";
									...
									<table id="example1" class="table table-striped table-bordered table-hover" width="100%" >
										...
										<tbody>
											...
											<?php
												$n=1;
                                                if(mysql_num_rows($q_gt) > 0) {
												    while($data = mysql_fetch_array($q_gt)) {
											...
											?>
    											    <tr <?= $warna?>>
        												...
        												<td> <a href="<?=base_url()?>?page=keuangan/gt_track&action=track&halaman= TRACK TOLAKAN GIRO&kode_gt=<?=$data['kode_gt']?>">
        													 <?php echo $data['kode_gt'];?></a>
                                                        </td>
...
$(document).ready(function (e) {
    $("#saveForm").on('submit',(function(e) {
        ...
        if(grand_total != 0 && grand_total > 0) {
            $(".animated").show();
            $.ajax({
                url: "<?=base_url()?>ajax/j_gt.php?func=save",
                type: "POST",
                ...
                success: function(html)
                {
                    var msg = html.split("||");
                    if(msg[0] == "00") {
                        window.location = '<?=base_url()?>?page=keuangan/gt&halaman= TOLAKAN GIRO&pesan='+msg[1];
                    } else {
                        notifError(msg[1]);
                    }
                    $(".animated").hide();
...
$("#batal_input").click(function(event) {
    ...
    document.getElementById('show_input_gt').style.display = "none";
  });
...
$(document).on('change', '#kode_cabang, #kode_pelanggan, #kode_supplier', function() {
	...
	$('#detail_input_gt').html('<tr><td colspan="9" class="text-center"> Tidak ada item barang. </td></tr>');
    $.ajax({
        type: "POST",
        url: "<?=base_url()?>ajax/j_gt.php?func=loaditemgiro",
	 ...
	 function BindSelect2() {
		 $('[name="kode_pg"]').prop('disabled', false);
		 $('[name="kode_pg"]').select2({
			 'width': '100%'
		 });
	 }
});

$(document).on('change', '#kode_pg', function() {
    var id_form        = $("#id_form").val();
    var kode_user 	   = $("#kode_user").val();
    var kode_cabang    = $("#kode_cabang").val();
    var kode_pg   = $("#kode_pg").val();
	var rad = $("[name='rad']:checked").val();
    $.ajax({
        type: "POST",
        url: "<?=base_url()?>ajax/j_gt.php?func=loaditemuser",
        data: "kode_user="+kode_user+"&kode_cabang="+kode_cabang+"&id_form="+id_form+"&rad="+rad+"&kode_pg="+kode_pg,
        cache:false,
        success: function(data) {
			...
			$('#detail_input_gt').html(data.table);
			numberjs();
         }
     });
});
...
function numberjs() {
	...
	$('[name="metode_dtl[]"]').select2({
		'width': '100%',
	});
}
//----------------------------END LOAD TABEL DETAIL--------------------------------//

$(document).on('change paste keyup', '[name="nominal_tolakan[]"]', function () {
	hitung($(this));
});

function hitung($this) {
	var
		$id = $this.attr('data-id'),
		$nominal_asli = $('#nominal_asli_' + $id).val() || 0,
		$nominal_tolakan = $('#nominal_tolakan_' + $id).val() || 0,
		$nominal_sisa = 0;
	
	if (parseFloat($nominal_tolakan) >= 1 && parseFloat($nominal_tolakan) <= parseFloat($nominal_asli)) {
		$nominal_sisa = parseFloat($nominal_asli - $nominal_tolakan);
		$('#text-nominal_sisa_' + $id).html($nominal_sisa);
		$('#text-nominal_sisa_' + $id).number(true, 2);
	} else {
		$('#nominal_tolakan_' + $id).val($nominal_asli);
		$('#text-nominal_sisa_' + $id).html(0);
		$('#text-nominal_sisa_' + $id).number(true, 2);
	}
}

$(document).on('change', '[name*="input_giro"]', function () {
	checkgiro($(this));
});
...
function checkgiro($this) {
	var
		...
		$nominal_asli = parseFloat($('#nominal_asli_' + $id).val() || 0);
	if ($input_giro.is(':checked')) {
		$('#input_cb_' + $id).val(1);
		$('#nominal_tolakan_' + $id).prop('disabled', false);
		$('#nominal_tolakan_' + $id).val($nominal_asli);
		$('#nominal_tolakan_' + $id).trigger('change');
	} else {
		$('#input_cb_' + $id).val(0);
		$('#nominal_tolakan_' + $id).val(0);
		$('#nominal_tolakan_' + $id).trigger('change');
		$('#nominal_tolakan_' + $id).prop('disabled', true);
	}
}
