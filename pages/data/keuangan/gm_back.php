					...
					<div class="panel-body">
                        <div class="form-horizontal">
                            <form action="" method="post" enctype="multipart/form-data" id="saveForm">
                                ...
                                <?php
									$res_hdr = mysql_fetch_array($gm_hdr_back); 
								?>		

			                        <div class="form-group">
			                         	<label class="col-lg-2 col-sm-2 control-label" style="text-align:left">Kode GM</label>
			                         	<div class="col-lg-4">
			                             	<input type="text" class="form-control" name="kode_gm" id="kode_gm" placeholder="Auto..." readonly value="<?=$res_hdr['kode_gm']?>">
			                         	</div>
				                    ...
				                    <div class="form-group">
                                        ...
                                        <div class="col-lg-4">
                                            <select id="kode_pelanggan" name="kode_pelanggan" class="select2" style="width: 100%;">
                                                <option value="<?php echo $res_hdr['kode_pelanggan'];?>"><?php echo $res_hdr['kode_pelanggan'].'&nbsp;&nbsp;||&nbsp;&nbsp;'.$res_hdr['nama_pelanggan'];?> </option>
                                                <?php 
                                                	(isset($res_hdr['id_gm_hdr']) ? $kode_pelanggan=$res_hdr['kode_pelanggan'] : $kode_pelanggan='');
                                                    while($row_pelanggan = mysql_fetch_array($q_pelanggan)) { ;?>
                                                    <option value="<?php echo $row_pelanggan['kode_pelanggan'];?>"
                                                    	<?php if($row_pelanggan['kode_pelanggan']==$kode_pelanggan){echo 'selected';} ?>>
                                            ...
                                            <table id="" class="table table-striped table-bordered table-hover" width="100%">
                                                ...
                                                <tbody id="detail_input_gm">
                                                	...
                                                	<tr>
														...
														<td>
															<input class="form-control" type="text" name="deskripsi[]" id="deskripsi[]" data-id="deskripsi" data-group="<?= $n ?>" value="<?=$res_dtl['deskripsi']?>"" readonly/>
														</td>
														<td>
															<input class="form-control" type="text" name="saldo_transaksi[]" id="saldo_transaksi[]" data-id="saldo_transaksi" data-group="<?= $n ?>" value="<?=$res_dtl['saldo_transaksi']?>" style="text-align:right" readonly/>
														</td>
														<td>
															<input class="form-control" type="text" name="nominal_bayar[]" id="nominal_bayar[]" data-id="nominal_bayar" data-group="<?= $n ?>" value="<?=$res_dtl['nominal_bayar']?>" style="text-align:right" autocomplete="off"/>
														</td>
														<td>
															<input class="form-control" type="text" name="nominal_pelunasan[]" id="nominal_pelunasan[]" data-id="nominal_pelunasan" data-group="<?= $n ?>" value="<?=$res_dtl['nominal_pelunasan']?>" style="text-align:right"/>
														</td>	
														<td>
									...
									<div class="form-group">
                                        ...
                                        <div class="col-lg-4"> 
                                            <select id="kode_coa_selisih" name="kode_coa_selisih" class="select2">
                                                ...
                                                <?php 
                                                	(isset($res_hdr['id_gm_hdr']) ? $kode_coa_selisih=$res_hdr['kode_coa_selisih'] : $kode_coa_selisih='');
									...
									<div class="form-group col-md-6">
                    					<?php 
											...
											<button type="submit" name="simpan" id="simpan" class="btn btn-primary" onclick="return confirm('Anda yakin akan menyimpan data Pengembalian Giro Masuk ini ?')"><i class="fa fa-check-square-o"></i> Simpan</button> 

                                            <?php } } } ?>
                                            
                                            <a href="<?=base_url()?>?page=keuangan/gm&halaman= GIRO MASUK" class="btn btn-danger" onclick="return confirm('Anda yakin akan keluar dari Form Pengembalian Giro Masuk ini ?          Data tidak akan disimpan jika anda keluar dari Form ini.')"><i class=" fa fa-reply"></i> Batal</a>
               						</div>
...
$(document).ready(function (e) {
    $("#saveForm").on('submit',(function(e) {
        var grand_total = parseInt($("#kode_gm").val());
        ...
        if(grand_total != 0 && grand_total > 0) {           
            $(".animated").show();
            $.ajax({
                url: "<?=base_url()?>ajax/j_gm.php?func=save_back",
                type: "POST",
                ...
                success: function(html)
                {
                    var msg = html.split("||");
                    if(msg[0] == "00") {
                        window.location = '<?=base_url()?>?page=keuangan/gm&halaman= GIRO MASUK&pengembalian='+msg[1];
                    } else {
                        notifError(msg[1]);
                    }
                    $(".animated").hide();
...
$(function(){
	...
	var kode_gm 	= $('#kode_gm').val();
	
	$.ajax({
        type: "POST",
        url: "<?=base_url()?>ajax/j_gm.php?func=addgirotarik",
        data: "bank_giro="+bank_giro+"&no_giro="+no_giro+"&tgl_giro="+tgl_giro+"&nominal="+nominal+"&id_form="+id_form+"&kode_gm="+kode_gm,
        cache:false,
