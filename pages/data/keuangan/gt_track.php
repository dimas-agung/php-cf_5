...
<section class="content">
    <div class="col-md-10">
      <?php
        $prev = mysql_fetch_array($q_gt_prev); {
        if (isset($prev['id_gt_hdr'])){
      ?>
          <a class="btn btn-warning" href="<?=base_url()?>?page=keuangan/gt_track&action=track&halaman= TRACK TOLAKAN GIRO&kode_gt=<?=$prev['kode_gt']?>">
      ...
      <?php
        ...
        $next = mysql_fetch_array($q_gt_next); {
        if (isset($next['id_gt_hdr'])){
      ?>
          &nbsp;<a class="btn btn-warning" href="<?=base_url()?>?page=keuangan/gt_track&action=track&halaman= TRACK TOLAKAN GIRO&kode_gt=<?=$next['kode_gt']?>">
      ...
      ?>
        ...
        <a href="<?=base_url()?>?page=keuangan/gt_back&action=pengembalian&halaman= PENGEMBALIAN TOLAKAN GIRO&kode_gt=<?=$stat['kode_gt']?>" class="btn pengembalian <?php echo $status2?>" style="color: white"><i class=" fa fa-retweet"></i> PENGEMBALIAN</a> -->
      </div>
      <div class="col-md-2">
        <a href="#modalAddItem" class="btn btn-info" data-toggle="modal"><i class=" fa fa-book"></i> JURNAL</a> 
        &nbsp;
        <a href="<?=base_url()?>?page=keuangan/gt&halaman= TOLAKAN GIRO" class="btn btn-danger pull-right" ><i class=" fa fa-reply"></i> BACK</a>
      </div>
</section>
...
<div class="box box-info">
    <div class="box-body">
     	&nbsp;
		<?php
			$res_hdr = mysql_fetch_array($q_gt_hdr); 
                                ...
                                <div class="panel-body">
                                	...
                                	<h3 style="text-align: center"><?php echo $status ?></h3> <hr style="border-top: 2px solid #5615157d;">
                                 	...
                                 	<div class="form-group">
			                         	...
			                         	<div class="col-lg-4">
			                             	<input type="text" class="form-control" name="kode_gt" id="kode_gt" placeholder="Auto..." readonly value="<?=$res_hdr['kode_gt']?>">
			                         	</div>
                         			 ...
                         			 <div class="form-group">
				                         ...
				                         <div class="col-lg-4">
				                             ...
				                             <input type="text" name="tgl_buat" id="tgl_buat" class="form-control" placeholder="Tanggal Tolakan Giro ..." value="<?=strftime("%A, %d %B %Y", strtotime($res_hdr['tgl_buat']))?>" autocomplete="off" disabled/>
                                              ...
                                              <table id="simple-table" class="table  table-bordered table-hover">
                                                <thead>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <th style="width:130px">Metode Pengganti</th>
                                                ...
                                                <tbody>
                                                    <?php
														$no = 1;                
                                                        while($res_dtl = mysql_fetch_array($q_gt_dtl)) {
															switch($res_dtl['metode_dtl']) {
																case 0:
																	$metode_dtl = 'Kas';
																	break;
																case 1:
        ...
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                 <h4 class="modal-title">Kode PG : <?=$res_hdr['kode_gt']?></h4>
            </div>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header" style="background-color: #9c0303">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                 <h4 class="modal-title" style="color: white">Pembatalan Kode PG : <?=$res_hdr['kode_gt']?></h4>
            </div>
            ...
            <form id="formPembatalan" method="post" enctype="multipart/form-data">             
                <div class="modal-body" style="min-height: 50px;">
                    <div class="control-group">
                        <div class="form-group">
			                ...
			                <input type="hidden" class="form-control" name="kode_gt_batal" id="kode_gt_batal" value="<?=$res_hdr['kode_gt']?>">
                ...
                <div class="modal-footer" style="background-color: #9a24241a; border-top: 1px solid #ab5d5d;">
                	<button type="submit" name="batal" id="batal" class="btn btn-default" onclick="return confirm('Anda yakin akan membatalkan Tolakan Giro ini ?')">
...
$(document).ready(function (e) {
    $("#formPembatalan").on('submit',(function(e) {
        ...
        if(alasan_batal != "") {           
            $(".animated").show();
            $.ajax({
                url: "<?=base_url()?>ajax/j_gt.php?func=pembatalan",
                type: "POST",
                ...
                success: function(html)
                {
                    var msg = html.split("||");
                    if(msg[0] == "00") {
                        window.location = '<?=base_url()?>?page=keuangan/gt&halaman= TOLAKAN GIRO&pembatalan='+msg[1];
                    } else {
                        notifError(msg[1]);
                    }
                    $(".animated").hide();
