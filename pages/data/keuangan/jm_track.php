<?php 	include "pages/data/script/jm.php"; 
...
<section class="content-header">
    <ol class="breadcrumb">
        ...
        <li><a href="#">Track Jurnal Memorial</a></li>
    </ol>
...
<section class="content">
      <div class="col-md-10">
      <?php
        $prev = mysql_fetch_array($q_jm_prev); {
        if (isset($prev['id_jm_hdr'])){
      ?>
          <a class="btn btn-warning" href="<?=base_url()?>?page=keuangan/jm_track&action=track&halaman= TRACK JURNAL MEMORIAL&kode_jm=<?=$prev['kode_jm']?>">
      ...
      <?php
        ...
        $next = mysql_fetch_array($q_jm_next); {
        if (isset($next['id_jm_hdr'])){
      ?>
          &nbsp;<a class="btn btn-warning" href="<?=base_url()?>?page=keuangan/jm_track&halaman= TRACK JURNAL MEMORIAL&action=track&kode_jm=<?=$next['kode_jm']?>">
    ...
    <div class="col-md-2">
      ...
      &nbsp;
      <a href="<?=base_url()?>?page=keuangan/jm&halaman= JURNAL MEMORIAL" class="btn btn-danger pull-right" ><i class=" fa fa-reply"></i> BACK</a>
    </div>
  </section>
<div class="box box-info">
    <div class="box-body">
		...
		<?php
			// HEADER
			$res_hdr = mysql_fetch_array($q_jm_hdr); 
		?>	
                            ...
                            <div class="panel panel-default">
                                <div class="panel-body">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-lg-10">
                                           <input type="text" required class="form-control" name="kode_jm" id="kode_jm" placeholder="Kode JM..." readonly value="<?=$res_hdr['kode_jm']?>">
                                        </div>
                                    </div>
                                    ...
                                    <label class="col-lg-2 col-sm-2 control-label" style="text-align:left">Tanggal JM</label>
                                      <div class="col-lg-10">
                                        <div class="input-group">
                                               <input type="text" name="tgl_buat" id="tgl_buat" class="form-control date-picker" autocomplete="off" value="<?=strftime("%A, %d %B %Y", strtotime($res_hdr['tgl_buat']))?>" disabled>
                                            ...
                                            <table id="" class="" rules="all">
                                                ...
                                                <tbody>
                                                    <?php
                                                        ...
                                                        $sub_debet = 0;
                                                        $sub_kredit = 0;
														                          while($res_dtl = mysql_fetch_array($q_jm_dtl)) { 
													                          ?>
                                                    	<tr>
                                                        <td style="text-align: center;"><?=$no++?></td>
                                                        <td><?=$res_dtl['kode_coa'].'&nbsp;&nbsp;||&nbsp;&nbsp;'.$res_dtl['nama_coa']?></td>
                                                        <td style="text-align: right;"><?= number_format($res_dtl['debet'], 2)?></td>
                                                        <td style="text-align: right;"><?= number_format($res_dtl['kredit'], 2)?></td>
                                                        <td><?=$res_dtl['keterangan_dtl']?></td>
                                                      </tr>

                                                    <?php 
                                                        $sub_debet += $res_dtl['debet'];
                                                        $sub_kredit += $res_dtl['kredit'];
                                                      } 
                                                    ?>  
                                                        <tr>
                                                            <td colspan="2" style="text-align:right"><b>Subtotal</b></td>
                                                            <td style="text-align:right ; font-weight:bold"><?= number_format($sub_debet, 2)?></td>
                                                            <td style="text-align:right; font-weight:bold"><?= number_format($sub_kredit, 2)?></td>
                                                            <td></td>
                                                        </tr>
                                                  </tbody>
                                                </table>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                 <h4 class="modal-title"><?=$res_hdr['kode_jm']?></h4>
            </div>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header" style="background-color: #9c0303">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                 <h4 class="modal-title" style="color: white">Pembatalan Kode JM : <?=$res_hdr['kode_jm']?></h4>
            </div>
            ...
            <form id="formPembatalan" method="post" enctype="multipart/form-data">             
                <div class="modal-body" style="min-height: 50px;">
                    <div class="control-group">
                      ...
                      <input type="hidden" class="form-control" name="kode_jm_batal" id="kode_jm_batal" value="<?=$res_hdr['kode_jm']?>">
                ...
                <div class="modal-footer" style="background-color: #9a24241a; border-top: 1px solid #ab5d5d;">
                  <button type="submit" name="batal" id="batal" class="btn btn-default" onclick="return confirm('Anda yakin akan membatalkan Jurnal Memorial ini ?')">
...
$(document).ready(function (e) {
    $("#formPembatalan").on('submit',(function(e) {
        ...
        if(alasan_batal != "") {           
            $(".animated").show();
            $.ajax({
                url: "<?=base_url()?>ajax/j_jm.php?func=pembatalan",
                type: "POST",
                ...
                success: function(html)
                {
                    var msg = html.split("||");
                    if(msg[0] == "00") {
                        window.location = '<?=base_url()?>?page=keuangan/jm&halaman= JURNAL MEMORIAL&pembatalan='+msg[1];
                    } else {
                        notifError(msg[1]);
                    }
                    $(".animated").hide();
